...
describe('issueRelations service', () => {
  ...
  it('delegates add and remove relation mutations', async () => {
    ...
    expect(api.post).toHaveBeenCalledWith('/issues/9/relations', { target_id: 2, type: 'blocks' })
    expect(api.delete).toHaveBeenCalledWith('/issues/9/relations', { target_id: 2, type: 'blocks' })
  })
})
