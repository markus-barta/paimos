...
- **Physical attacker with disk access.** A `$DATA_DIR` reader can read SQLite directly. PAIMOS doesn't encrypt-at-rest the DB itself; operators wanting that must use encrypted storage (LUKS, eCryptfs, etc.).
