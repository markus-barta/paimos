...
`scripts/deploy.sh ppm <tag>` takes a backup before every image swap. The backup step lives in `deploy::run` in `scripts/_deploy-lib.sh` and is non-skippable: a deploy that can't produce a valid tarball **does not proceed**.
