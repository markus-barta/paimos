...
| **CI scanner pipeline** ([`SECURITY_REVIEW.md`](SECURITY_REVIEW.md)) | gitleaks (blocking) + npm audit (blocking) + govulncheck (blocking) + gosec zero-baseline gate (blocking; baseline emptied by PAI-223) cover the SAST / dep / secret matrix | scanners catch known-pattern defects, not novel ones |
| **Former second-operator reference deployment** | an independent operator ran the secondary instance until June 2026 — the closest thing to external review the project has had: every deploy that succeeded without intervention validated the runbook for someone who didn't write it | not adversarial; didn't probe for vulnerabilities |
