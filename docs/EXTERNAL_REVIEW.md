...
| **CI scanner pipeline** ([`SECURITY_REVIEW.md`](SECURITY_REVIEW.md)) | gitleaks (blocking) + npm audit (blocking) + gosec + govulncheck (advisory pending PAI-223 / PAI-224) cover the SAST / dep / secret matrix | scanners catch known-pattern defects, not novel ones |
