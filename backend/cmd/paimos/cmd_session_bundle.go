...
//
// PAI-348 — `Source` is a CLI-side annotation only (never persisted
// server-side). When present it tags the entry as inherited from
// another project so downstream agents can disambiguate "rule from
// upstream paimos" from "rule authored on this project". `omitempty`
// keeps own entries free of empty annotation noise.
...
func resolveBundle(c *Client, project projectSummary, agentName string) (*bundlePayload, error) {
	...
	ownMemory := filterMemory(memRaw, currentUserID, agentEnvs)
