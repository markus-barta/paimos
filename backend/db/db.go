...
func migrate(db *sql.DB) error {
	...
	migrations := []migration{
		...
		//
		// Schema notes:
		//  - Composite primary key (user_id, device_id, project_id) so
		//    the toggle survives the user creating multiple devices on
		//    the same project + the same user toggling on multiple
		//    projects + multiple users sharing a device. ON DELETE
		//    CASCADE follows the user/project — orphan rows here would
		//    keep streaming to gone devices.
		//  - device_id is opaque TEXT (UUID generated by the CLI on
		//    first watch and stored at ~/.paimos/device-id). Capped at
		//    64 chars application-side to match the agent-attr cap;
		//    server is permissive about the format so future
		//    changes (e.g. host:cpu fingerprint) don't need a schema
		//    change.
		//  - enabled is INTEGER (0/1) — SQLite booleans are integers,
		//    matching the convention from M85/M91/M92 etc.
		//  - PAI-342 picks M97; this migration is M98 (skipping leaves
		//    a hole — that's fine, schema_versions tolerates gaps).
		//
		// PAI-341 (knowledge-plane sync) reuses this table verbatim:
		// one (user, device, project) row covers ALL kinds for that
		// triple. Per-kind opt-in would require a kinds_mask column —
		// deliberately deferred until there's evidence users want
		// it. The simpler "watch everything for this project" matches
		// the sync engine's "register all kinds" iteration in
		// `paimos sync init`.
		{98, []string{
